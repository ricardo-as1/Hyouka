...
module.exports = {
  ...
  async run(client, message) {
    ...
    const embed = createBotInfoEmbed(client, message.author, default_prefix);
    ...
    waiter(sentMsg, message.author.id, commands, default_prefix);
  },
};
  ...
  collector.on("collect", async (interaction) => {
    if (interaction.customId === "help-menu") {
      ...
      const embed = new EmbedBuilder()
        .setTitle(`Comandos - ${selectedCategory.charAt(0).toUpperCase() + selectedCategory.slice(1)}`)
        .setColor(DefaultEmbedColor)
        .setDescription(`Selecione os comandos da categoria **${selectedCategory.charAt(0).toUpperCase() + selectedCategory.slice(1)}**.`);
    ...
    .setDescription(
      currentCommands
        .map(cmd => `
          ...
          **Permissões:** ${cmd.permissions.length > 0 ? cmd.permissions.join(", ") : "Nenhuma"}
        `)
        .join("\n")
    );
